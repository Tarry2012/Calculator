...
class Calculate{
	...
	//判断空格的合法性，第一部合法性判断
		...
		LinkedList<Character> stack = new LinkedList<Character>();   //申请栈
		char stackTop = ' ';             //栈顶元素
		stack.addFirst('#');           //先将“#”压栈
		StringBuilder expression = new StringBuilder(Expression);   //接收参数
		int[] temp = new int[expression.length()];
		int t = 0;         //记录左括弧的个数
		String newString = "";          //用来记录截得的字符串
		int count = 0;                //计算,个数
		for (int i = 0; i < expression.length(); i++){
			temp[i] = 0;           //初始化，用来记录所有左括弧的下标
		}
		
		for (int i = 0; i < expression.length(); i++){
//			System.out.println("wwwwwi = " + i + "当前下标的字符 = " + expression.charAt(i));
			...
			if (i >= 0 && (expression.charAt(i) == '(')){
				//如果字符为‘（’并且它的前一个字符是小写字母
				temp[t++] = i;          //得到一个左括弧的下标
		//		if (i >= 2){
		//			if (expression.indexOf())
		//		}
//				System.out.println("压栈i = " + i + "当前length = " + expression.length());
				stack.addFirst(expression.charAt(i));      //左括弧入栈
			}

			if (expression.charAt(i) == ')'){
//				System.out.println("出栈 i = " + i+"当前length = " +expression.length());
				stackTop = stack.peekFirst();     //获取栈顶元素
				stack.removeFirst();           //出栈
//				System.out.println("llalalal");
//				System.out.println("好expression = " + expression);
//				System.out.println("remoce   " + stackTop+ "temp[t01] = " + temp[t - 1]);
				if (temp[t - 1] >= 2){              //当前左括弧前面的字符串长度大小
					if ((temp[t - 1] >= 2) && (expression.indexOf("ln(", temp[t - 1] - 3) != -1)){

						System.out.println("#");
						newString = expression.substring(temp[t - 1] - 2, i + 1);
						for (int k = 0; k < newString.length(); k ++){
							if (newString.charAt(k) == ','){
								count ++;
							}
							if (count == 0 && k == newString.length() - 1){
					//			count = 0;
								expression.replace(temp[t-1]-2,i+1,"");   //将newString这段字符替换成空
...
//								continue;
					...
					} else if ((temp[t - 1] >= 3) && ((expression.indexOf("sin(", temp[t - 1] -4) != -1) || 
					 	 (expression.indexOf("cos", temp[t - 1] - 4) != -1) || 
						 (expression.indexOf("tan(", temp[t - 1] - 4) != -1) ||
						 (expression.indexOf("exp(", temp[t - 1] - 4) != -1))){

						newString = expression.substring(temp[t - 1] - 3, i + 1);
						...
						for (int k = 0; k < newString.length(); k++){
							...
							if (count != 0){
								System.out.println("三角函数参数过多，error");
								System.exit(1);
							}
							if (count == 0 && k == newString.length() - 1){
					//			count = 0;
								expression.replace(temp[t - 1] - 3, i +1 ,"");
								i = temp[t - 1] -4;
								t--;
								System.out.println("expression2 = " + expression);
//								System.out.println("t-1 = " + temp[t - 1]);
//								continue;
							}
						}
						if (expression.length() == 0){
					...
					} else if ((temp[t-1] >= 3) && ((expression.indexOf("mod(", temp[t-1] - 4) != -1) 
							||(expression.indexOf("log(", temp[t-1] - 4) != -1) || 
							(expression.indexOf("pow(", temp[t - 1] - 4) != -1) 
						/*	(expression.indexOf("var(", temp[t-1] - 4) != -1) || 
							(expression.indexOf("sum(", temp[t-1] - 4) != -1) || 
							(expression.indexOf("arg(", temp[t-1] - 4) != -1)*/)) {
...
//									System.out.println("count = " + count);
								...
								if (count == 1 && k == newString.length()-1){
								//count = 0;
									expression.replace(temp[t-1]-3, i+1, "");
									i = temp[t - 1] - 4;
									t--;
//									System.out.println("pow()函数");
									System.out.println("expression = " + expression);
								}
							}
//						System.out.println("count" + count);
						if (count != 1 && (((newString.substring(0, 4)).compareTo("mod(") == 0) || 
							((newString.substring(0, 4)).compareTo("pow(") == 0) ||
							((newString.substring(0, 4)).compareTo("log(") == 0))){
								System.out.println("三角函数参数过多或过少，error");
								System.exit(1);
						} 
						if (expression.length() == 0){
							break;
					...
					} else if ((temp[t -1] >= 3) && (expression.indexOf("var(", temp[t-1] - 4) != -1) || 
							(expression.indexOf("sum(", temp[t-1] - 4) != -1) || 
							(expression.indexOf("arg(", temp[t-1] - 4) != -1)){

					/*		if (newString.charAt(k) == ')' && (newString.substring(0,4).compareTo("sum()") == 0) ||
									(newString.substring(0,4).compareTo("arg(") ==0) ||
								(newString.substring(0,4).compareTo("var(") == 0)){
					*/		
							newString = expression.substring(temp[t - 1] - 3, i + 1);
//							System.out.println("~~~~~~~s");
							System.out.println("newString = " +newString);
							for (int k = 0; k < newString.length(); k++){		
							if ((k == newString.length()-1) && ((newString.substring(0,4).compareTo("sum(") == 0) ||
									(newString.substring(0,4).compareTo("arg(") ==0) ||
								(newString.substring(0,4).compareTo("var(") == 0))){
								if (newString.charAt(k - 1) != ']'){
									System.out.println("数组中的参数错误，error");
									System.exit(1);
								}
								count = 0;
								expression.replace(temp[t - 1] - 3, i+1,"");
					...
					newString = expression.substring(temp[t-1], i+1);
					System.out.println("newString = " +newString);
					for (int k = 0; k < newString.length(); k++){
								if (newString.charAt(k) == ','){
									count ++;
								...
								if (count > 0 && k == newString.length() - 1){
									System.out.println("非函数，非数组中存在逗号，error!");
									System.exit(1);
								}
								if (count == 0 && k == newString.length() - 1){
									expression.replace(temp[t - 1], i+1, "");
									i = temp[t-1];
									t--;
								}
							}
			...
			if (expression.length() != 0){
				for(int k = 0; k < expression.length() - 1; k++){
					...
					if (k == newString.length() - 1){
						expression.replace(0, k, "");
					}
				}
				if (count > 0){
					System.out.println("表达式中存在多个无归所逗号, error");
					System.exit(1);
				}
			}
...
class Calculate{
	...
	//判断空格的合法性，第一部合法性判断
		boolean JudgeSpace() { 
			...
			for (int i = 0; i < Expression.length(); i ++) {
				//对出现空格的地方，前后字符进行解析，判断空格的存在是否合法
				if (Character.isWhitespace(Expression.charAt(i)) && i != 1 && (i + 1) <= Expression.length()){
			...
			for (int i = 0; i < Expression.length(); i++){
				...
				if (Expression.charAt(i) == ','){        
					...
					if (Expression.charAt(i + 1) == ',' || Expression.charAt(i + 1) == ')' || Expression.charAt(i + 1) == ']'){
						...
						System.out.println("逗号运算符。。。多了,error!");
						return false;
					}
					...
					if (stackTop == '['){
						...
						count++;
						if ((Expression.indexOf("w(", temp[t-1]- 2) != -1) || (Expression.indexOf("d(", temp[t-1] - 2) != -1) 
								||(Expression.indexOf("g(", temp[t-1] - 2) != -1)){
							if (count > 1){         
							...
							System.out.println("逗号运算符错误,参数过多");
							return false;
						}
					}
				}
				...
				if (Expression.charAt(i) == ')'){
					...
					if (stackTop == '('){     //如果栈顶元素是'（',则t--，并且让count = 0
						...
						if ((Expression.indexOf("w(", temp[t-1]- 2) != -1) || (Expression.indexOf("d(", temp[t-1] - 2) != -1) 
							...
							if (count != 1){        
								//对于pow(),mode(),log()这三个函数，如果出现三个及三个以上参数，报错，并返回false
								System.out.println("运算符错误! 逗号过多或者过少");
								return false;
							}
							t--;    //括号匹配，t--记录（号的下标前移
							continue;
...
//				System.out.println("number1 = " + number1 + "number2 = " + number2);
		...
		for(int i = 0; i < expression.length(); i++ ){
			...
			if( expression.charAt(i) == '-' ){ 
				...
				if( i == 0 ){
					...
					expression.insert(i, "(0");
					i += 2;
					/*通过while循环找到一个完整的数字(只能由数字、小数点、e组成)*/
					while(Character.isDigit(expression.charAt(i+1)) || expression.charAt(i+1) == '.' || expression.charAt(i+1) == 'e')
						i++;
					/*插入')'*/
					expression.insert(++i, ')');
					i = k;	
				...
				/*a--b -> a-(0-b)*/
					...
					k = i;
					/*如果'-'的下一个字符为'-'，则在该负数前后加上(）将字符串变成a-(0-b)*/
					if(expression.charAt(i+1) == '-'){
						expression.delete(i+1, i+2);
					}
					/*插入"（0"*/
			...
			/*如果遇到')'，且后一个字符是数字或'('或字母（函数名），则添加乘号*/
			if(expression.charAt(i) == ')' && (Character.isDigit(expression.charAt(i+1)) || expression.charAt(i+1) == '(' || Character.isLowerCase(expression.charAt(i+1)))){
				i++;
				expression.insert(i, '*');
			}		
			/*如果遇到字母（函数名），且前一个字符是数字，则添加乘号*/
	...
	public double calculateTrans(ArrayDeque<String> queueTrans){
		...
		if(queueTrans.size() == 1 ){
			/*读出队列中的唯一元素并将String类型的数转换为double类型*/
			tempDouble = Double.parseDouble(queueTrans.removeFirst());
			/*将最后结果转换为String类型*/
			resultFinalExpression = Double.toString(tempDouble);
			/*输出结果*/
			System.out.println("f:"+resultFinalExpression);
		...
		/*如果队列中元素>=2个，则依次读出处理*/
		while(!queueTrans.isEmpty()){
			tempString = queueTrans.removeFirst(); //每次读出一个字符串到tempString
			/*如果读出的字符串是一个数字，则直接压到ArrayCalcu的队头*/
	...
	public Double DealTriangle(String str){
		...
		for(i = 2; i < 7; i++){   //该方法处理的最短函数名为3个字符，最长的三角函数名为6个字符
			if(str.charAt(i) == '(')
				break;
		}
		...
		switch(tempFuncName){
			/*将函数的参数传入中缀转后的方法解析，将解析的结果传入计算方法进行计算，计算的结果则为sin函数的参数代入计算*/
			case "sin": 
				return Math.sin(calculateTrans(analysisExpression(tempExpression)));
			/*将函数的参数传入中缀转后的方法解析，将解析的结果传入计算方法进行计算，计算的结果则为cos函数的参数代入计算*/
			case "cos":
				return Math.cos(calculateTrans(analysisExpression(tempExpression)));
			/*将函数的参数传入中缀转后的方法解析，将解析的结果传入计算方法进行计算，计算的结果则为tan函数的参数代入计算*/
			case "tan":
				return Math.tan(calculateTrans(analysisExpression(tempExpression)));
			/*将函数的参数传入中缀转后的方法解析，将解析的结果传入计算方法进行计算，计算的结果则为sinh函数的参数代入计算*/
			case "sinh":
				return Math.sinh(calculateTrans(analysisExpression(tempExpression)));
			/*将函数的参数传入中缀转后的方法解析，将解析的结果传入计算方法进行计算，计算的结果则为cosh函数的参数代入计算*/
			case "cosh":
				return Math.cosh(calculateTrans(analysisExpression(tempExpression)));
			/*将函数的参数传入中缀转后的方法解析，将解析的结果传入计算方法进行计算，计算的结果则为tanh函数的参数代入计算*/
			case "tanh":	
				return Math.tanh(calculateTrans(analysisExpression(tempExpression)));
			case "arcsin":
			/*将函数的参数传入中缀转后的方法解析，将解析的结果传入计算方法进行计算，计算的结果存入tempdouble*/
				tempdouble = calculateTrans(analysisExpression(tempExpression));
				/*如果tempdouble不符合该函数的定义域，则会报错*/
				if(Double.toString(Math.asin(tempdouble)).equals("NaN")){
					System.out.println("操作数不正确");
					System.exit(1);
				}
				/*返回计算结果*/
	...
	public ArrayDeque<String> analysisExpression(String transExpression){
		...
		for (int i = 0; i < length; i++){
			//解析出数字，如果当前字符为数字、小数点、科学计数法e则存入num中
			//辨析e是表示科学计数法还是一个函数，则通过判断它的下一个字符是否为字母
			if (Character.isDigit(transExpression.charAt(i))|| transExpression.charAt(i) == '.' || (transExpression.charAt(i) == 'e' && !Character.isLowerCase(transExpression.charAt(i + 1)))){				
				num = num.append(transExpression.charAt(i));
				//如果当前字符的下一个字符不是数字也不是点也不是e则可以截取该字符串，该字符串表示一个完整的数
				if ((i + 1 <= length) && ((Character.isDigit(transExpression.charAt(i+1)) == false) && transExpression.charAt(i+1) != '.')&& (transExpression.charAt(i+1) != 'e') ){
					//直接将数入队列
					queueTrans.addLast(num.toString());
			        string.append(num);   //delete~~~~~~~~~~~~~~~~`
					string.append(" ");	  //delete~~~~~~~~~~~~~~~~
   ...
   public double DealStatistics(String str){
		...
		for (int i = 0; i < funcInfor[2].length(); i++){
			...
			//以逗号为分割符，并且判断该逗号是否为一个嵌套统计函数里的逗号，
			//如果是，则该逗号不是分割符
			if (funcInfor[2].charAt(i) == ',' && between == 0){
				queue.addLast(numDivide.toString());
				numDivide.delete(0, numDivide.length());
				continue;
			}
...
>>>>>>> 987f03ddaee2fa3c8a3b5b7ba49fdcf3c29c1b83
